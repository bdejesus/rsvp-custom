...
function rsvp_frontend_main_form($attendeeID, $rsvpStep = "handleRsvp") {
	...
	$attendee = $wpdb->get_row($wpdb->prepare("SELECT id, firstName, lastName, email, rsvpStatus, note, kidsMeal, additionalAttendee, veggieMeal, personalGreeting, numberOfGuests
  ...
  $maxGuests = $attendee->numberOfGuests;
  ...
  if($attendeeID <= 0) {
    ...
    $form .= rsvp_BeginningFormField("", "").
      "<label for=\"attendeeFirstName\">".__("First Name: ", 'rsvp-plugin')."</label><br/>".
      "<div class='placeholder-value'>".htmlspecialchars($rsvp_saved_form_vars['attendeeFirstName'])."</div>".
      "<input type=\"text\" name=\"attendeeFirstName\" id=\"attendeeFirstName\" />".
      RSVP_END_FORM_FIELD;
    $form .= RSVP_END_PARA;

    $form .= RSVP_START_PARA;
    $form .= rsvp_BeginningFormField("", "").
      "<label for=\"attendeeLastName\">".__("Last Name: ", 'rsvp-plugin')."</label><br/>".
      "<div class='placeholder-value'>".htmlspecialchars($rsvp_saved_form_vars['attendeeLastName'])."</div>".
      "<input type=\"text\" name=\"attendeeLastName\" id=\"attendeeLastName\" />".
      RSVP_END_FORM_FIELD;
    $form .= RSVP_END_PARA;
  }
  ...
  if(get_option(OPTION_RSVP_HIDE_EMAIL_FIELD) != "Y") {
    $form .= rsvp_BeginningFormField("", "rsvpBorderTop").
      RSVP_START_PARA."<label for=\"mainEmail\">".__("Email Address", 'rsvp-plugin')."</label>".RSVP_END_PARA.
        "<div class='placeholder-value'>".htmlspecialchars($attendee->email)."</div>".
        "<input type=\"text\" name=\"mainEmail\" id=\"mainEmail\" />".
      RSVP_END_FORM_FIELD;
  }
  ...
  /*$sql = "SELECT id, firstName, lastName FROM ".ATTENDEES_TABLE."
	...
	if(count($associations) > 0) {
		$form .= "<h6>".__("The following people are associated with you.  At this time you can RSVP for them as well.", 'rsvp-plugin')."</h6>";
      ...
      if($a->id != $attendeeID) {
        ...
        $form .= rsvp_BeginningFormField("", "").
          "<label for=\"attendeeFirstName\">".__("First Name: ", 'rsvp-plugin')."</label><br/>".
          "<div class='placeholder-value'>".htmlspecialchars($a->firstName)."</div>".
          "<input type=\"text\" name=\"attendeeFirstName\" id=\"attendeeFirstName\" />".
          RSVP_END_FORM_FIELD;
        $form .= rsvp_BeginningFormField("", "").
          "<label for=\"attendeeFirstName\">".__("Last Name: ", 'rsvp-plugin')."</label><br/>".
          "<div class='placeholder-value'>".htmlspecialchars($a->lastName)."</div>".
          "<input type=\"text\" name=\"attendeeFirstName\" id=\"attendeeFirstName\" />".
          RSVP_END_FORM_FIELD;
    ...
    $form .= "<script type='text/javascript'>
      jQuery(document).ready(function($){
        var i = 0;
        while (i < ".$availableGuests.") {
          handleAddRsvpClick();
          i++;
        }

        var formFields = $('.rsvpFormField .placeholder-value');
        $.each(formFields, function() {
          var formObj = $(this);
          var fieldValue = formObj.text();
          formObj.next().val(fieldValue);
          formObj.hide();
        })
    ...
    $sub_text = __("You can invite an additional ".$availableGuests." guests.", 'rsvp-plugin');
    ...
    $form .= "<div id=\"additionalRsvpContainer\">\r\n
                <input type=\"hidden\" name=\"additionalRsvp\" id=\"additionalRsvp\" value=\"".$maxGuests."\" />".
              "</div>";
...
function rsvp_find(&$output, &$text) {
  ...
  $passcodeOnlyOption = (rsvp_require_only_passcode_to_register()) ? true : false;
	...
	if(!$passcodeOnlyOption && ((strlen($_REQUEST['firstName']) <= 1) || (strlen($_REQUEST['lastName']) <= 1))) {
		$output = "<div class=\"rsvpParagraph\" style=\"color:red\">".__("A first and last name must be specified", 'rsvp-plugin')."</div>\r\n";
			...
			if(count($attendees) > 0) {
				...
				foreach($attendees as $a) {
					$output .= "<form method=\"post\" action=\"$rsvp_form_action\">\r\n
									...
									<div class=\"rsvpParagraph\" style=\"text-align:left;\">\r\n
...
function rsvp_inject_add_guests_js($attendeeID) {
  ...
  if(get_option(OPTION_RSVP_NUM_ADDITIONAL_GUESTS) != "") {
	...
	$form = "<script type=\"text/javascript\" language=\"javascript\">\r\n
                      ...
                      \"	<label for=\\\"newAttending\" + numAdditional + \"FirstName\\\">".__("Person's first name", 'rsvp-plugin')."&nbsp;</label><br/>\" + \r\n
												\"  <input type=\\\"text\\\" name=\\\"newAttending\" + numAdditional + \"FirstName\\\" id=\\\"newAttending\" + numAdditional + \"FirstName\\\" />\" + \r\n
									  	\"</div>\" + \r\n
											\"<div class=\\\"rsvpFormField\\\">\" + \r\n
                      \"	<label for=\\\"newAttending\" + numAdditional + \"LastName\\\">".__("Person's last name", 'rsvp-plugin')."</label><br/>\" + \r\n
